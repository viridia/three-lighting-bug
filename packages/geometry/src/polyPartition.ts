/** Functions to decompose a complex polygon into convex polys.

    Taken from https://github.com/x6ud/poly-partition-js (with modifications suggested by the
    original author):

    * Handle case where hole is touching the outer edge.
    * Change the definition of a Point to match that used by polygon-clipping: a 2-tuple instead
      of an object with x and y properties.
 */
...
function isConvexTolerance(p1: Point, p2: Point, p3: Point): boolean {
  return area(p1, p2, p3) <= 0;
}

// Whether point is inside a corner of a counterclockwise polygon.
function inCone(p1: Point, p2: Point, p3: Point, p: Point): boolean {
  const convex = isConvexTolerance(p1, p2, p3);
  if (convex) {
    return isConvexTolerance(p1, p2, p) && isConvexTolerance(p2, p3, p);
  } else {
    return isConvexTolerance(p1, p2, p) || isConvexTolerance(p2, p3, p);
  }
}
...
function intersects(p11: Point, p12: Point, p21: Point, p22: Point) {
  ...
  const v1ortX = p12[1] - p11[1];
  const v1ortY = p11[0] - p12[0];
  const v2ortX = p22[1] - p21[1];
  const v2ortY = p21[0] - p22[0];

  const v21X = p21[0] - p11[0];
  const v21Y = p21[1] - p11[1];
  const dot21 = v21X * v1ortX + v21Y * v1ortY;
  const v22X = p22[0] - p11[0];
  const v22Y = p22[1] - p11[1];
  const dot22 = v22X * v1ortX + v22Y * v1ortY;

  const v11X = p11[0] - p21[0];
  const v11Y = p11[1] - p21[1];
  const dot11 = v11X * v2ortX + v11Y * v2ortY;
  const v12X = p12[0] - p21[0];
  const v12Y = p12[1] - p21[1];
  ...
  while (holes.length) {
    ...
    for (let i = 0; i < polyLen; ++i) {
      ...
      if (polyPointIndex >= 0) {
        ...
        const v1x = polyPoint[0] - holePoint[0];
        const v1y = polyPoint[1] - holePoint[1];
        const v1Len = Math.sqrt(v1x * v1x + v1y * v1y);
        const v2x = bestPoint[0] - holePoint[0];
        const v2y = bestPoint[1] - holePoint[1];
