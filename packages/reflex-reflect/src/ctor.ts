 ...
 */
export function classType<Cls extends object, Ser = JsonObject>(
  options: Omit<IClassTypeDescriptor<Cls, Ser>, 'kind'>
): IClassTypeDescriptor<Cls, Ser> {
  return { kind: 'class', ...options };
}

/** Custom type allows app-specific type extensions. */
export function customType<T, Ser = JsonValue>(
  options: Omit<ICustomTypeDescriptor<T, Ser>, 'kind'>
): ICustomTypeDescriptor<T, Ser> {
