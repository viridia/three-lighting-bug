...
export interface ITypeDescriptor<T> {
  ...
  /** Used to select a more specialized property editor. */
  readonly subtype?: string;
  readonly custom?: ICustomTypeOptions;
  readonly __dummy?: T; // Needed because we have to reference `T` somewhere in the template.
}
...
export interface IStringTypeDescriptor<T extends string = string> extends ITypeDescriptor<T> {
  kind: 'string';
  maxLength?: number;
  multiline?: boolean; // Editing hint.
  enumVals?: T[] | (() => T[]);
 ...
 */
export interface IClassTypeDescriptor<Cls extends object, Ser = JsonObject>
  extends ITypeDescriptor<Cls> {
  kind: 'class';
  name: string;
  properties: IPropertyDescriptors<Cls>;
  additionalPropertes?: boolean; // Whether extra properties are allowed - default true.
  supertype?: IClassTypeDescriptor<object>;
  ...
  marshal?: <T extends Cls>(value: T) => Ser;
  unmarshal?: (ser: Ser) => Cls;
}

/** Custom primitive type - an app-specific type which is known by name. Editing requires
    the app to supply an editor component.
 */
export interface ICustomTypeDescriptor<T, Ser = JsonValue> extends ITypeDescriptor<T> {
  kind: 'custom';
  name: string;
  clone: (value: T) => T;
