  ...
  switch (a.kind) {
    ...
    case 'integer': {
      ...
      return (
        ai.minVal === bi.minVal &&
        ai.maxVal === bi.maxVal &&
        equal(ai.subtype, bi.subtype) &&
        equal(ai.custom, bi.custom) &&
        equal(ai.enumVals, bi.enumVals) &&
        equal(ai.bitmask, bi.bitmask)
      );
    }
    ...
    case 'float': {
      ...
      return (
        af.minVal === bf.minVal &&
        af.maxVal === bf.maxVal &&
        equal(af.custom, bf.custom) &&
        equal(af.subtype, bf.subtype)
      );
    }
    ...
    case 'tuple': {
      ...
      if (!equal(a.custom, b.custom) || !equal(a.subtype, b.subtype)) {
        return false;
      }
    ...
    case 'custom': {
      ...
      return ca.name === cb.name && equal(ca.subtype, cb.subtype);
    }
    default:
      return false;
  }
