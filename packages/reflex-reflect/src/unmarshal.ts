...
import { unmarshallStruct } from './struct';
  ...
  switch (desc.kind) {
    ...
    case 'struct': {
      ...
      if (value && typeof value === 'object' && !Array.isArray(value)) {
        ...
        const result = unmarshallStruct(d.properties, value);
        if (d.migrate) {
          return d.migrate(result);
        }
        return result;
      }
      throw new DeserializationError(`Expected object, got ${typeof value}`);
    ...
    case 'class': {
      const d = desc as IClassTypeDescriptor<any, any>;
